...
const MEDIATHEQUE_CARD_VERSION = '2.3.0';
...
function generateCode128Svg(text, height) {
  height = height || 80;
  if (!text) return '';
  var codes = [];
  var startCode = 104;
  ...
  var barWidth = 2;
  var width = binary.length * barWidth;
...
class MediathequeCard extends HTMLElement {
  ...
  _renderBookRow(loan, enabledBadges, showEmprunteur) {
    ...
    if (loan.extend_disabled) {
      badgesHtml += `<span class="badge-days" style="color:#b71c1c;background:#ffcdd2">✗ Désactivé</span>`;
  ...
  _render() {
    // Guards (pattern mini-graph-card)
    if (!this._config || !this._hass || !this._config.entity) return;
    ...
    if (mode === 'due') {
      ...
      const sorted = [].concat(livres).sort(function(a, b) {
        return (a.days_left != null ? a.days_left : 0) - (b.days_left != null ? b.days_left : 0);
      });
      ...
      for (const member of sortedMembers) {
        ...
        const sorted = [].concat(loans).sort(function(a, b) {
          return (a.days_left != null ? a.days_left : 0) - (b.days_left != null ? b.days_left : 0);
        });
