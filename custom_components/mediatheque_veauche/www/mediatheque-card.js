...
`;var pt={overdue:"En retard",today:"\xC0 rendre aujourd'hui",urgent:"1 \xE0 3 jours restants",soon:"4 \xE0 7 jours restants",ok:"Plus de 7 jours",not_extendable:"D\xE9j\xE0 prolong\xE9"},ut={entity:"Entit\xE9 (sensor)",title:"Titre personnalis\xE9",mode:"Mode",badges:"Filtres par badge",total_entity:'Entit\xE9 du total (mode "due")',card_id:"Identifiant carte"},J=class extends y{constructor(){super(...arguments);this._config={entity:""}}setConfig(t){this._config=t??{entity:""}}createRenderRoot(){return this}render(){if(!this.hass)return u``;let t=[{name:"entity",required:!0,selector:{entity:{domain:"sensor",integration:"mediatheque_veauche"}}},{name:"title",selector:{text:{}}},{name:"mode",selector:{select:{mode:"dropdown",options:[{value:"all",label:"Tous les emprunts (par membre)"},{value:"due",label:"\xC0 rendre cette semaine"}]}}},{name:"badges",selector:{select:{multiple:!0,options:D.map(r=>({value:r,label:pt[r]??r}))}}},{name:"total_entity",selector:{entity:{domain:"sensor",integration:"mediatheque_veauche"}}},{name:"card_id",selector:{text:{}}}];return u`
    ...
    `}_matchesBadgeFilter(t,r){return r.includes(ve(t.days_left).type)}_renderDue(t,r,s,o){let d=(t.attributes??{}).livres??[],a=s?d.filter(f=>this._matchesBadgeFilter(f,r)):d,c=this._totalEntityState??this._hass?.states[this._config.entity.replace("_due_week","_total")],p=c?parseInt(c.state,10)||0:null,l=(c?.attributes?.card_id??this._config?.card_id??"")||"",m=p!==null?`${a.length} / ${p}`:`${a.length}`,g=a.length>0,$=[...a].sort((f,v)=>(f.days_left??0)-(v.days_left??0));return u`
      <ha-card>
        ${this._renderHeader(o,m,g,l)}
        ${$.length===0?u`<div class="empty-state">Aucun livre à rendre cette semaine</div>`:$.map(f=>this._renderBookRow(f,!0))}
        ${this._renderModals(l)}
      </ha-card>
    `}_renderAll(t,r,s,o){let n=t.attributes??{},d=n.membres??{},a=n.compte??"",c=n.card_id??this._config?.card_id??"",p={},l=0;for(let[f,v]of Object.entries(d)){let U=s?v.filter(oe=>this._matchesBadgeFilter(oe,r)):v;U.length>0&&(p[f]=U,l+=U.length)}let m=Object.keys(p).sort((f,v)=>f===a?-1:v===a?1:f.localeCompare(v)),g=`${l} emprunt${l>1?"s":""}`,$=s&&l>0;return u`
      <ha-card>
        ${this._renderHeader(o,g,$,c)}
        ${m.length===0?u`<div class="empty-state">Aucun emprunt en cours</div>`:m.map(f=>{let v=p[f];if(!v)return h;let U=f===a?"\u{1F464}":"\u{1F466}",oe=[...v].sort((ne,Ve)=>(ne.days_left??0)-(Ve.days_left??0));return u`
    ...
    `}};b.styles=[ze,Ie],x([P({attribute:!1})],b.prototype,"hass",1),x([T()],b.prototype,"_config",2),x([T()],b.prototype,"_detailLoan",2),x([T()],b.prototype,"_confirmExtend",2),x([T()],b.prototype,"_barcodeOpen",2);var be=b;je();window.loadCardHelpers?.();customElements.get("mediatheque-card")||customElements.define("mediatheque-card",be);window.customCards=window.customCards??[];window.customCards.some(i=>i.type==="mediatheque-card")||window.customCards.push({type:"mediatheque-card",name:"M\xE9diath\xE8que de Veauche",description:"Affiche les emprunts de la m\xE9diath\xE8que de Veauche"});export{be as MediathequeCard};
