...
`;var gt={overdue:"En retard",today:"\xC0 rendre aujourd'hui",urgent:"1 \xE0 3 jours restants",soon:"4 \xE0 7 jours restants",ok:"Plus de 7 jours",not_extendable:"D\xE9j\xE0 prolong\xE9"},vt={entity:"Entit\xE9 (sensor)",title:"Titre personnalis\xE9",mode:"Mode",badges:"Filtres par badge",total_entity:'Entit\xE9 du total (mode "due")',card_id:"Identifiant carte"},Z=class extends _{constructor(){super(...arguments);this._config={entity:""}}setConfig(t){this._config=t??{entity:""}}createRenderRoot(){return this}render(){if(!this.hass)return u``;let t=[{name:"entity",required:!0,selector:{entity:{domain:"sensor",integration:"mediatheque_veauche"}}},{name:"title",selector:{text:{}}},{name:"mode",selector:{select:{mode:"dropdown",options:[{value:"list",label:"Liste (group\xE9e par membre)"},{value:"covers",label:"Couvertures (grille \xE0 rendre)"}]}}},{name:"badges",selector:{select:{multiple:!0,options:R.map(r=>({value:r,label:gt[r]??r}))}}},{name:"total_entity",selector:{entity:{domain:"sensor",integration:"mediatheque_veauche"}}},{name:"card_id",selector:{text:{}}}];return u`
    ...
    `}_renderList(t,r,s,o){let n=t.attributes??{},d=n.membres??{},a=n.compte??"",c=n.card_id??this._config?.card_id??"",p={},l=0;for(let[b,y]of Object.entries(d)){let B=s?y.filter(de=>this._matchesBadgeFilter(de,r)):y;B.length>0&&(p[b]=B,l+=B.length)}let m=Object.keys(p).sort((b,y)=>b===a?-1:y===a?1:b.localeCompare(y)),f=`${l} emprunt${l>1?"s":""}`,g=s&&l>0;return u`
      <ha-card>
        ${this._renderHeader(o,f,g,c)}
        ${m.length===0?u`<div class="empty-state">Aucun emprunt en cours</div>`:m.map(b=>{let y=p[b];if(!y)return h;let B=b===a?"\u{1F464}":"\u{1F466}",de=[...y].sort((ce,Ge)=>(ce.days_left??0)-(Ge.days_left??0));return u`
    ...
    `}};E.styles=[We,Fe],x([H({attribute:!1})],E.prototype,"hass",1),x([M()],E.prototype,"_config",2),x([M()],E.prototype,"_detailLoan",2),x([M()],E.prototype,"_confirmExtend",2),x([M()],E.prototype,"_barcodeOpen",2);Ve();window.loadCardHelpers?.();customElements.get("mediatheque-card")||customElements.define("mediatheque-card",E);window.customCards=window.customCards??[];window.customCards.some(i=>i.type==="mediatheque-card")||window.customCards.push({type:"mediatheque-card",name:"M\xE9diath\xE8que de Veauche",description:"Affiche les emprunts de la m\xE9diath\xE8que de Veauche"});export{E as MediathequeCard};
